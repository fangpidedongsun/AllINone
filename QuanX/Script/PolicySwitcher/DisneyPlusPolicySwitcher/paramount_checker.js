...
const $ = new Env('Paramount+ 解锁检测')
let disneyPolicyName = $.getval('Helge_0x00.Paramount_Policy') || 'Paramount'
let debug = $.getval('Helge_0x00.Paramount_Debug') === 'true'
let retry = $.getval('Helge_0x00.Paramount_Retry') === 'true'
let t = parseInt($.getval('Helge_0x00.Paramount_Timeout')) || 8000
let sortByTime = $.getval('Helge_0x00.Paramount_Sort_By_Time') === 'true'
let concurrency = parseInt($.getval('Helge_0x00.Paramount_Concurrency')) || 10
...
;(async () => {
  ...
  if (!isValidPolicy(policies[disneyPolicyName])) {
    ...
    $.setval(disneyPolicyName, 'Helge_0x00.Paramount_Policy')
  }
  ...
  $.setval(JSON.stringify(availablePolicies), 'Helge_0x00.Paramount_Available_Policies')
})()
  .catch(error => {
    console.log(error)
    ...
    for (let { policy, status, region, time } of results) {
      switch (status) {
        case STATUS_COMING: {
          ...
          console.log(`${policy}: Paramount+ 即将登陆 ➟ ${flag}${regionName}`)
          break
        }
        case STATUS_AVAILABLE: {
          let flag = getCountryFlagEmoji(region) ?? ''
          let regionName = REGIONS?.[region.toUpperCase()]?.chinese ?? ''
          console.log(`${policy}: 支持 Paramount+ ➟ ${flag}${regionName}`)
          availablePolicies.push({ policy, region, time })
          break
        }
        case STATUS_NOT_AVAILABLE:
          console.log(`${policy}: 不支持 Paramount+`)
          break
        case STATUS_TIMEOUT:
...
function lookupTargetPolicy(policies = {}) {
  let policyNames = Object.entries(policies)
    .filter(([key, val]) => key.search(/Paramount\+|Paramount Plus|派拉蒙/gi) !== -1)
    .map(([key, val]) => key)
  if (policyNames.length === 1) {
    return policyNames[0]
  } else if (policyNames.length <= 0) {
    throw '没有找到 Paramount+ 策略组，请在 BoxJS 中填写正确的策略组名称'
  } else {
    throw `找到多个 Paramount+ 策略组，请在 BoxJS 中填写正确的策略组名称`
  }
}
