...
获取步骤：向机器人发送 /start 后，机器人会告诉一个API URL，例如：https://dianbao.vercel.app/send/abc，其中的 abc 就是API密匙。"
...
POST_TG(){
	Get_IP
	if [[ -z "${NAME}" ]]; then
		wget -qO- --post-data="text=\`[疑似被墙警告]\`  —  \[\`${IP}\`]&parse_mode=Markdown&disable_notification=false"  "https://dianbao.vercel.app/send/${TOKEN}" >> ${LOG_file}
	else
		wget -qO- --post-data="text=\`[疑似被墙警告]\`  —  \[${NAME}] (\`${IP}\`)&parse_mode=Markdown&disable_notification=false"  "https://dianbao.vercel.app/send/${TOKEN}" >> ${LOG_file}
	fi
	echo "" >> ${LOG_file}
}
Get_IP(){
